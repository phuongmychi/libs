...
export default class App extends React.Component {
  ...
  calculateSinResult =  () =>{
    const text = this.state.resultText
    var n = Math.sin(text * Math.PI/180);

    // do magic
    this.setState({
      calculationText: Math.round(n * 100) / 100
    })
  }
  ...
  calculateCosResult =  () =>{
    const text = this.state.resultText
    var n = Math.cos(text * Math.PI/180);
  ...
  calculateTanResult =  () =>{
    const text = this.state.resultText
    var n = Math.tan(text * Math.PI/180);
  ...
  buttonPressed(text){
    if(text == '='){
      ...
      return this.calculateSinResult()

    }else if(text == 'cos'){
      return this.calculateCosResult()
    }else if(text == 'tan')
    {
      return this.calculateTanResult()

    }else if(text == '√'){
    
    return this.calculateSQResult();
